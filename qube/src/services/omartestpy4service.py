...
class omartestpy4Service:
    ...
    def find_by_id(self, entity_id):
        ...
        data = omartestpy4.query.get(entity_id)
        if data is None:
            raise omartestpy4ServiceError(
                'omartestpy4 {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = omartestpy4.query.filter(
            omartestpy4.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = omartestpy4.query.get(entity_id)  # omartestpy4 is a mongo class
        if record is None:
            raise omartestpy4ServiceError(
                'omartestpy4 {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
